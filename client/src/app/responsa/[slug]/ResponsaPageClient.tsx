...
interface ResponsaPageClientProps {
  initialResponsa?: Responsa | null;
  slug: string;
}

export default function ResponsaPageClient({ initialResponsa, slug }: ResponsaPageClientProps) {
  const [responsa, setResponsa] = useState<Responsa | null>(initialResponsa || null);
  const [commentsData, setCommentsData] = useState<CommentType[]>([]);
  const [isLoading, setIsLoading] = useState(!initialResponsa);

  const loadResponsa = useCallback(async () => {
    if (initialResponsa) {
      setResponsa(initialResponsa);
      setCommentsData(initialResponsa.comments || []);
      setIsLoading(false);
      return;
    }
