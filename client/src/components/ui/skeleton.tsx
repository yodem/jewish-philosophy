  ...
  return (
    <div className={cn("flex flex-col w-full h-auto items-center transition-shadow duration-200 overflow-hidden bg-gradient-to-br from-gray-100 to-white dark:from-gray-800 dark:to-gray-900 rounded-lg shadow-md", className)}>
      ...
      <div className="w-full overflow-hidden rounded-t-lg relative aspect-[4/3]">
        <Skeleton className="w-full h-full bg-muted dark:bg-secondary" />
      </div>
      ...
      <div className="flex flex-col items-center p-4 w-full space-y-3">
        ...
        <div className="w-full space-y-1">
          <Skeleton className="h-5 w-4/5 bg-muted dark:bg-secondary mx-auto" />
          <Skeleton className="h-5 w-3/5 bg-muted dark:bg-secondary mx-auto" />
        </div>
        
        {/* Episode count (optional) */}
        <Skeleton className="h-3 w-20 bg-muted dark:bg-secondary mx-auto" />
        
        {/* Description */}
        <div className="w-full space-y-1">
          <Skeleton className="h-3 w-full bg-muted dark:bg-secondary" />
          <Skeleton className="h-3 w-4/5 bg-muted dark:bg-secondary mx-auto" />
        </div>
        
        {/* Button */}
        <Skeleton className="h-10 w-full bg-muted dark:bg-secondary rounded-md mt-4" />
      </div>
    </div>
  )
}
...
function TermCardSkeleton({ className }: { className?: string }) {
  return (
    <div className={cn("w-full bg-card dark:bg-background rounded-lg shadow-md p-4 space-y-4 h-full flex flex-col", className)}>
      {/* Title */}
      <div className="space-y-2">
        <Skeleton className="h-6 w-4/5 bg-muted dark:bg-secondary" />
        <Skeleton className="h-4 w-3/4 bg-muted dark:bg-secondary" />
      </div>
      ...
      <div className="space-y-2 flex-grow">
        <Skeleton className="h-3 w-full bg-muted dark:bg-secondary" />
        <Skeleton className="h-3 w-full bg-muted dark:bg-secondary" />
        <Skeleton className="h-3 w-2/3 bg-muted dark:bg-secondary" />
        <Skeleton className="h-3 w-4/5 bg-muted dark:bg-secondary" />
      </div>

      {/* Author */}
      <div className="space-y-1">
        <Skeleton className="h-3 w-1/3 bg-muted dark:bg-secondary" />
      </div>

      {/* Categories */}
      <div className="flex gap-1 flex-wrap justify-end">
        <Skeleton className="h-5 w-12 bg-muted dark:bg-secondary rounded-full" />
        <Skeleton className="h-5 w-16 bg-muted dark:bg-secondary rounded-full" />
        <Skeleton className="h-5 w-14 bg-muted dark:bg-secondary rounded-full" />
      </div>
    </div>
  )
}
