...
export function Subscribe({
  ...
  return (
    <section className="max-w-3xl mx-auto px-6 py-12 bg-white rounded-lg shadow-md flex flex-col md:flex-row items-center gap-8">
      <div className="flex-1 mb-6 md:mb-0">
        <h4 className="text-2xl font-semibold mb-2">{headline}</h4>
        <p className="text-gray-700">{content}</p>
      </div>
      <form className="flex flex-col sm:flex-row items-stretch gap-3 flex-1 w-full max-w-md" action={formAction}>
        <input
          name="email"
          type="email"
        ...
        <button
          type="submit"
          className="px-6 py-2 bg-cyan-500 hover:bg-cyan-600 text-white font-semibold rounded-md transition"
        >
          {buttonText}
        </button>
      </form>
